...
// Inicializar materiales con su estado is_required
const initialMaterials = (props.section.materials || []).map(m => ({
    id: m.id,
    is_required: m.pivot?.is_required || false
}));
...
const selectedMaterialsDetails = computed(() => {
    return form.materials
        .map(mat => {
            const material = props.materials?.find(m => m.id === mat.id);
